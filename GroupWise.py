...
class WelcomePage(customtkinter.CTkFrame):
    def __init__(self, parent, controller, **kwargs):
        ...
        try:
            ...
            image_path = os.path.join(script_dir, "WelcomePageLogo.jpg")
            img = Image.open(image_path)
            self.logo_image = customtkinter.CTkImage(light_image=img, dark_image=img, size=(150, 200))
            logo_label = customtkinter.CTkLabel(self, image=self.logo_image, text="")
            logo_label.pack(pady=(50, 10))
        except (FileNotFoundError, Exception) as e:
            print(f"Error loading logo: {e}")
            customtkinter.CTkLabel(self, text="[Logo Not Found]", font=("Arial", 16)).pack(pady=(50,10))

        customtkinter.CTkLabel(self, text="Welcome to GroupWise!", font=("Arial", 20, "bold")).pack(pady=(10,10))
        customtkinter.CTkLabel(self, text="Group work just got smarter!", font=("Arial", 12)).pack(pady=(0,20))
        customtkinter.CTkButton(self, text="GET STARTED", font=("Arial", 11, "bold"), command=lambda: controller.show_frame("InputPage"), width=140, height=40).pack()
...
class InputPage(customtkinter.CTkFrame):
    def __init__(self, parent, controller, **kwargs):
        ...
        customtkinter.CTkLabel(self, text="Enter Member and Tasks", font=("Arial", 20, "bold")).pack(pady=(30,0))
        customtkinter.CTkLabel(self, text="Enter each name or task on a new line.", font=("Arial", 10)).pack(pady=(0,15))
        ...
        customtkinter.CTkLabel(member_frame, text="Members", font=("Arial", 12, "bold")).pack()
        self.member_text = customtkinter.CTkTextbox(member_frame, width=250, height=250, font=("Arial", 11))
        ...
        customtkinter.CTkLabel(task_frame, text="Tasks", font=("Arial", 12, "bold")).pack()
        self.task_text = customtkinter.CTkTextbox(task_frame, width=250, height=250, font=("Arial", 11))
        ...
        customtkinter.CTkLabel(group_frame, text="Number of Groups (Optional)", font=("Arial", 12, "bold")).pack()
        self.group_entry = customtkinter.CTkEntry(group_frame, width=140, font=("Arial", 11))
        ...
        customtkinter.CTkButton(button_frame, text="BACK", command=lambda: controller.show_frame("WelcomePage"), fg_color="#d3d3d3", text_color="black", hover_color="#c0c0c0", font=("Arial", 14)).grid(row=0, column=0, padx=15)
        customtkinter.CTkButton(button_frame, text="ASSIGN TASKS", command=self.assign_tasks, font=("Arial", 14)).grid(row=0, column=1, padx=15)
...
class ResultPage(customtkinter.CTkFrame):
    def __init__(self, parent, controller, **kwargs):
        ...
        customtkinter.CTkLabel(self, text="Task Assignments", font=("Arial", 20, "bold")).pack(pady=25)

        table_frame = customtkinter.CTkFrame(self, fg_color="transparent")
        table_frame.pack(pady=10, padx=20)

        columns = ("Group", "Member", "Task")
        self.result_table = ttk.Treeview(table_frame, columns=columns, show="headings", height=15)
        ...
        self.result_table.column("Member", width=250)
        self.result_table.column("Task", width=350)
        ...
        # Bottom Buttons
        bottom_buttons = customtkinter.CTkFrame(self, fg_color="transparent")
        bottom_buttons.pack(pady=20)

        customtkinter.CTkButton(bottom_buttons, text="Back", command=lambda: controller.show_frame("InputPage"), fg_color="#d3d3d3", text_color="black", hover_color="#c0c0c0", font=("Arial", 14)).grid(row=0, column=0, padx=10)
        customtkinter.CTkButton(bottom_buttons, text="Save", command=self.save_results, fg_color="#28a745", hover_color="#218838", font=("Arial", 14)).grid(row=0, column=1, padx=10)
        customtkinter.CTkButton(bottom_buttons, text="Re-randomize", command=lambda: controller.frames["InputPage"].assign_tasks(), fg_color="#ffc107", text_color="black", hover_color="#e0a800", font=("Arial", 14)).grid(row=0, column=2, padx=10)
        customtkinter.CTkButton(bottom_buttons, text="Exit", command=controller.quit, fg_color="#d9534f", hover_color="#c82333", font=("Arial", 14)).grid(row=0, column=3, padx=10)
